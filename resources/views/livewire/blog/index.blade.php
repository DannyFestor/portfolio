                ...
                <section class="flex-1">
                    ...
                    <section class="flex flex-wrap justify-end gap-2">
                        @foreach($post->tags as $tag)
                            <a href="{{ $tag->url }}" class="px-2 py-1 rounded flex items-center gap-1" style="color: {{ $tag->text_color }}; background-color: {{ $tag->background_color }}; border: 1px solid {{ $tag->border_color }}">
                                <img class="h-6 w-6" src="{{ asset('icons/' . $tag->logo ) }}" alt="">
                                {{ $tag->title }}
                            </a>
                        @endforeach
