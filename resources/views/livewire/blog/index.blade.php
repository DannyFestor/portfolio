    ...
    @foreach($posts as $post)
        <article class="flex flex-col bg-white p-4 rounded shadow" role="article">
            ...
            <section class="mt-2 prose font-serif">
                {!! nl2br(Str::limit($post->description, 200)) !!} <a href="{{ route('blog.show', $post) }}" class="text-xs text-blue-600 font-sans no-underline hover:underline">(... Continue Reading)</a>
            </section>
        </article>
    @endforeach
    </section>
